...
def vec_select_sum(veclist, k, D):
    ...
    True
    '''
    # return Vec(D, {k:sum(v[k] for v in veclist if v[k] == 0) for k in D})
...
def scale_vecs(vecdict):
    ...
    [True, True]
    '''
    return [Vec(v.D, {d:(1/k)*v[d] for d in v.D}) for k, v in vecdict.items()]
...
def GF2_span(D, L):
    ...
    for i in range(2**len(L)):
        binary_rep = bin(i)[2:]
        binary_rep = '0'*(len(L)-len(binary_rep)) + binary_rep
        binary_rep = [int(element) for element in reversed(binary_rep)]
        summation = sum([L[j] for j in range(len(L)) if binary_rep[j]==1])
        if summation:

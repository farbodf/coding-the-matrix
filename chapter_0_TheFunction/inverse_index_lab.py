...
def movie_review(name):
    ...
    """
    reviews = ["See it!", "A gem!", "Meh", "Ideological claptrap!"]
    return reviews[random.randint(0, 3)]
...
def orSearch(inverseIndex, query):
    ...
    for word in inverseIndex:
        if word in query:
            indexes.update(inverseIndex[word])
    return indexes
...
def andSearch(inverseIndex, query):
    ...
    True
    """
    indexes = inverseIndex[query[0]].copy()
    for i in range(1, len(query)):
        indexes.intersection_update(inverseIndex[query[i]])
    return indexes
